...
# After loading SKILLS_TAXONOMY
if not SKILLS_TAXONOMY:
    raise RuntimeError("skills_taxonomy.json is empty – please populate it with at least one skill.")
...
def normalize_skills(raw_skills: list[str], threshold: int = 80) -> list[str]:
    """Normalize a list of raw skill strings, dropping unrecognized or invalid ones."""
    normalized = []
    for s in raw_skills:
        if not isinstance(s, str) or not s.strip():
            continue          # skip None, numbers, empty strings
        norm = normalize_skill(s.strip(), threshold)
